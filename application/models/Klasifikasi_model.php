...
class Klasifikasi_model extends CI_Model {
    ...
    public function get_parameter($parameter,$val_param,$prediksi,$akurasi)
    {
    	$sql = "
    		SELECT (
    			select count(prediksi) 
    			from data_tes 
    			where `$parameter`= '$val_param' AND prediksi='$prediksi' AND akurasi='$akurasi'
	...
	public function get_parameter1($parameter,$val_param,$prediksi)
    {
    	$sql = "
    		SELECT (
    			select count(prediksi) 
	...
	public function get_data_by($id)
	{
		...
		$this->db->where('MONTH(tanggal)', $bulan_sekarang);
		$this->db->where('YEAR(tanggal)', $tahun_sekarang );
		if($this->db->get('data_set')->result()){
			return TRUE;
		} else {
			return FALSE;
		}
